...
function addToCart(event) {
    ...
    let ItemImg = shopItem.getElementsByClassName('shop-item-image')[0].src
...
function addItemToCart(title, price, ItemImg) {
    ...
    for (let i = 0; i < cartItemName.length; i++) {
        if (cartItemName[i].innerText == title) {
            alert(`You have alrady added ${title}`)
            return
        }
    }
...
newItem.innerHTML= newCartRow
    cartItems.appendChild(newItem)
newItem.getElementsByClassName('btn-danger')[0].addEventListener('click',removeCartItems)
newItem.getElementsByClassName('cart-quantity-input')[0].addEventListener('change', quantityChanged)
}
